  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        <p className="text-surface-500">Failed to load notification statistics</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-surface-100 p-5">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-lg bg-marsana-50 text-marsana-600">
              <Bell className="w-5 h-5" />
            </div>
            <div>
              <p className="text-2xl font-bold text-surface-900">{inApp.total || 0}</p>
              <p className="text-sm text-surface-500">Total In-App</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-surface-100 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-surface-900">{inApp.unread || 0}</p>
        ...
        <div className="bg-white rounded-xl border border-surface-100 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-surface-900">{logs.total || 0}</p>
              <p className="text-sm text-surface-500">Total Sent</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-surface-100 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-surface-900">{logs.sent_today || 0}</p>
              <p className="text-sm text-surface-500">Sent Today</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-surface-100 p-6">
          <h3 className="text-lg font-semibold text-surface-900 mb-4">By Channel</h3>
          <div className="space-y-3">
            {Object.entries(logs.by_channel || {}).map(([channel, count]) => (
              <div key={channel} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {channel === 'email' && <Mail className="h-4 w-4 text-blue-500" />}
                  {channel === 'in_app' && <Bell className="h-4 w-4 text-marsana-500" />}
                  {channel === 'push' && <Smartphone className="h-4 w-4 text-purple-500" />}
                  <span className="text-sm text-surface-700 capitalize">{channel.replace('_', ' ')}</span>
                </div>
        ...
        <div className="bg-white rounded-xl border border-surface-100 p-6">
          <h3 className="text-lg font-semibold text-surface-900 mb-4">By Status</h3>
          <div className="space-y-3">
            {Object.entries(logs.by_status || {}).map(([status, count]) => (
              <div key={status} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-surface-700 capitalize">{status}</span>
                </div>
                <span className="text-sm font-medium text-surface-900">{count}</span>
              </div>
            ))}
      ...
      {inApp.by_type && Object.keys(inApp.by_type).length > 0 && (
        <div className="bg-white rounded-xl border border-surface-100 p-6">
          <h3 className="text-lg font-semibold text-surface-900 mb-4">In-App Notifications by Type</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {Object.entries(inApp.by_type).map(([type, count]) => (
              <div key={type} className="text-center p-3 bg-surface-50 rounded-lg">
